...
public class SimuOrderAOImpl implements ISimuOrderAO {
    ...
    public BigDecimal minCountLimt = new BigDecimal("0.00001");

    public BigDecimal maxCountLimt = new BigDecimal("1000000");
    ...
    public String submit(XN650050Req req) {
        ...
        } else if (!ESimuOrderType.MARKET.getCode().equals(req.getType())) {
            throw new BizException(EBizErrorCode.DEFAULT.getCode(), "不支持的委托类型");
        }

        // 委托数量是否超过限制
        BigDecimal count = StringValidater.toBigDecimal(req.getTotalCount());
        if (count.compareTo(minCountLimt) >= 0
                && count.compareTo(maxCountLimt) <= 0) {

            BigDecimal[] results = count.divideAndRemainder(minCountLimt);
            System.out.println(results[0] + "");
            System.out.println(results[1] + "");

            // 是否是最小委托数量的整数倍
            if (BigDecimal.ZERO
                .compareTo(count.divideAndRemainder(minCountLimt)[1]) != 0) {
                throw new BizException(EBizErrorCode.DEFAULT.getCode(),
                    "委托数量应当0.00001的整数倍");
            }
    ...
    private SimuOrder submitBuyOrder(XN650050Req req) {
        ...
        // 落地委托单
        SimuOrder simuOrder = saveSimuOrder(req, totalCount, price,
            totalAmount);
    ...
    private SimuOrder saveSimuOrder(XN650050Req req, BigDecimal totalCount,
        ...
        simuOrderBO.saveSimuOrder(simuOrder);
        return simuOrder;
    }
    ...
    private void saveHandicap(SimuOrder simuOrder) {
        ...
        if (ESimuOrderType.LIMIT.getCode().equals(simuOrder.getType())) {
            ...
            handicapBO.saveHandicap(handicap);
